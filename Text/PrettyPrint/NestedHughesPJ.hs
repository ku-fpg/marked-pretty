-----------------------------------------------------------------------------
-- |
-- Module      :  Text.PrettyPrint.NestedHughesPJ
...
--
-- Nested added by Andy Gill, Oct 08.
--
-----------------------------------------------------------------------------
...
{-
        AJG: Adding open, close, and group, which scope/nest a pretty printed text.
        These make no difference to the output text, but allow sub-expressions
        to be denoted. So, for example, keywords can be bold. This nesting
...
string_txt (Open _) s2   = s2
string_txt (Close)  s2   = s2
...
group :: String -> Doc -> Doc
group name doc = open name <> doc <> close name

open  name = textBeside_ (Open name)  0 Empty
close name = textBeside_ Close 0 Empty
